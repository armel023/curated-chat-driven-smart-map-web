...
import {
  // searchPlaceNearbyToImport,
  searchPlaceTextQuery,
} from "@/actions/search-import-action";
...
export default function ImportSearchView() {
  ...
  const handleSearch = async () => {
    ...
    const searchedData: SearchPlaceTextQueryResult = await searchPlaceTextQuery(
      searchQuery,
      pageToken,
    );
    console.log("Search results:", searchedData);
    setPageToken(searchedData.nextPageToken || "");
    setSearchResults(searchedData.places);
    setSearching(false);
    // Mock search results
    // setTimeout(() => {
